...
class IntradayCsvBarPriceHandler(AbstractBarPriceHandler):
    """
    IntradayCsvBarPriceHandler is designed to read intraday CSV
    files with Open-High-Low-Close-Volume (OHLCV) data for each
    requested financial instrument and stream those to the
    provided events queue as BarEvents.
    """
    def __init__(
        self, csv_dir, events_queue,
        init_tickers=None, period=60,
    ...
    def stream_next(self):
        ...
        bev = self._create_event(index, self.period, ticker, row)
        # Store event
...
if __name__ == '__main__':
    ...
    handler = IntradayCsvBarPriceHandler(
        csv_dir=csv_dir, events_queue=events_queue,
        init_tickers=init_tickers,
    )

    for index, row in handler.bar_stream:
        print(index, row)
